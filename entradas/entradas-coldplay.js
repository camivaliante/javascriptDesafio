import { cantidad, fechasColdplay, sectoresColdplay } from "../artistas.js";

// FECHA SELECT OPTION
  const fechasC = document.getElementById("fechaColdplay");

  fechasColdplay.forEach(fechaColdplay => {
    const option = document.createElement("option");
    option.value = fechaColdplay.dia;
    option.innerHTML = fechaColdplay.dia;

    fechasC.appendChild(option);

  })

// SECTOR SELECT OPTION
  const sectoresC = document.getElementById("sectoresC");

  sectoresColdplay.forEach(sectorColdplay => {
    let option = document.createElement("option");
    option.value = sectorColdplay.precio;
    option.text = sectorColdplay.nombre;

    sectoresC.appendChild(option);
...
// COMPRAR
  ...
  let totalCompraColdplay = [];

  const formcoldplay = document.getElementById('form-coldplay');

  formcoldplay.addEventListener('submit', (e) => {

     e.preventDefault();

     const dia = document.getElementById('fechaColdplay').value;
     const precio = document.getElementById('sectoresC').value;
     ...
     totalCompraColdplay.push({dia, cant, totalCs, sector});
     ...
     function comprar(){
      ...
      const enJSON = JSON.stringify(totalCompraColdplay);

      localStorage.setItem('totalCompraColdplay', enJSON);

      const miPedidoColdplay = JSON.parse(localStorage.getItem('totalCompraColdplay'));
